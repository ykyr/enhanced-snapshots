            ...
            while (true) {
                ...
                for (TaskEntry taskEntry : tasks) {
                    if (taskEntry.getStatus().equalsIgnoreCase("waiting")) {
                        ...
                        Task task = new AWSBackupVolumeTask(awsCredentialsProvider, taskEntry.getVolume(), routineInstanceId, new WorkerConfiguration(configuration), taskEntry.getInstanceId());
                        task.execute();
                        try {
